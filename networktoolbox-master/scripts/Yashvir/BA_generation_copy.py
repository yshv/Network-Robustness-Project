...
def ba_generation_distributed(nodes, m, edge_step, edge_count, graph_num):
    ...
    nt.Database.insert_graph(graph, "Topology_Data", "robustness-sim-test-test", node_data=True, use_pickle=True, type="BA",
    ...
    for i in range(1, edge_count+1):
        ...
        while nx.is_connected(graph_copy) != True or min([d for n, d in graph_copy.degree()])<=2:
            ...
            graph_copy = copy.deepcopy(graph)
            graph_copy.remove_edges_from(choices)
        nt.Database.insert_graph(graph_copy, "Topology_Data", "robustness-sim-test-test", node_data=True, use_pickle=True, type="BA",
                                 timestamp=datetime.utcnow(), graph_num=graph_num, edge_removal_count=edge_count, edge_removal_step=edge_step,
...
if __name__ == "__main__":
    ...
    for i in range(0, graph_num):
        ba_generation_distributed(nodes, 5, edge_step, edge_count, i)
        print(i)
