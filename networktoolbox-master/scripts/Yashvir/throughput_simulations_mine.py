...
if __name__== "__main__":
    ...
    ray.init()
    graph_list = nt.Database.read_topology_dataset_list("Topology_Data", "robustness-sim-basic", find_dic={"lambda_r":{"$exists":False}})
    pb = nt.Tools.ProgressBar(len(graph_list))
    actor = pb.actor
    tasks = [static_ilp_distributed.remote(graph, _id, db="Topology_Data", collection="robustness-sim-basic", actor=actor, max_time=3600, threads=num_cpus) for graph, _id in graph_list]
    pb.print_until_done()
    ray.get(tasks)
