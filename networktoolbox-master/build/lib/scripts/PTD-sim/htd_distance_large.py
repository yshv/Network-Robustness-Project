...
@ray.remote
def htd_network_design(graph, E, T_c, N_sub, alpha,length_limit,gamma,write = False, collection="HTD-distance"):
    """
...
#     exit()
    ...
    length_rest = length_limit - np.array([design_graph[s][d]["weight"]*80 for s,d in design_graph.edges]).sum()
    E_rest = np.diagonal(edge_matrix).sum()
    print('L_intra: {}'.format(length_rest))
    print('E_intra: {}'.format(E_rest))
    
    
    design_tasks = [
        GA_run.remote(len(subnetwork), edge_matrix[i, i], extract_T_c_subnetwork(subnetwork, T_c, mappings[i]), alpha,
                      subnetwork, topology_num = 1, gamma = 0, write = False,length_limit = length_rest*edge_matrix[i, i]/E_rest) for i, subnetwork in enumerate(subnetworks)]
#     GA_run(N,E, T_C,alpha,grid_graph,topology_num,write,gamma,length_limit)
...
#     top = nt.Topology.Topology()
    ...
    DWC_structure = nt.Tools.get_demand_weighted_cost([[design_graph,0]], [T_c], alpha, penalty_num=1000)[0]
    DWC_distance = nt.Tools.get_demand_weighted_cost_distance([[design_graph, 1]], [T_c], alpha)[0]
    Alpha = design_graph.number_of_edges()/N/(N-1)*2
    DWC_combined = Alpha*DWC_distance + (1-Alpha)* DWC_structure

    if write == True:
        topology_data = nt.Tools.graph_to_database_topology(design_graph)
        node_data = nt.Tools.node_data_to_database(dict(design_graph.nodes.data()), use_pickle=True)
        nt.Database.insert_graph(design_graph, "Topology_Data", collection, node_data=True, use_pickle=True,
                                     type = "HTD-distance", T_c=T_c, DWC = DWC_combined,alpha=alpha, topology_data = topology_data,gamma = gamma, length_limit = length_limit)

    
    
    return design_graph,DWC_combined
...
#         print(self.E)
        ...
        self.top = nt.Topology.Topology()
        self.solution_graph = None
        self.algorithm_param =  {'max_num_iteration': 100,
                                   'population_size':100,
                                   'mutation_probability':0.1,
                                   'elit_ratio': 0.01,
                                   'crossover_probability': 0.8,
...
#     print(indeces)
    # Run all the ray instances
    results = ray.get([func.remote(N,E, T_C,alpha,grid_graph,indeces[i+1]-indeces[i],write, gamma,length_limit) for i in range(workers)])
    return results
    
@ray.remote
def GA_run(N,E, T_C,alpha,grid_graph,topology_num,write,gamma,length_limit):
    
    Solutions = []
    for i in range(topology_num):
        ptd = PTD(N,E, T_C,alpha,grid_graph,length_limit)
        solution = ptd.run_ga()
        ...
        if write == True:
            ...
            nt.Database.insert_graph(graph, "Topology_Data", "prufer-select-ga", node_data=True, use_pickle=True,
                                     ...
                                     gamma=gamma,length_limit = length_limit)
...
if __name__ == "__main__":
    ...
    for gamma in gamma_value:
        ...
        #                                             find_dic={"FF-kSP Capacity": {'$exists': True},'gamma':gamma, 'nodes': 100},
        #                                             max_count=1)
        # T_c = dataset["T_c"][0]
    ...
    #     print(T_c)
        ...
        for i in tqdm(range(topology_num)):
            tasks.append(htd_network_design.remote(graph, E, T_c, N_sub, alpha,length_limit,gamma,write = True,
                                                   collection="HTD-distance-revised"))
    results = ray.get(tasks)
        
        
        # print('N: {}'.format(len(designed_graph)))
        # print('E: {}'.format(designed_graph.number_of_edges()))
        # print('length_limit: {}'.format(length_limit))
        # print('total fiber length: {}'.format(np.array([designed_graph[s][d]["weight"]*80 for s,d in designed_graph.edges]).sum()))
        # print('DWC_combined: {}'.format(DWC_combined))
